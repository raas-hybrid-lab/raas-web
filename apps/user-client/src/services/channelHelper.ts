...
class ChannelHelper {
    ...
    private async _initializeClients(): Promise<void> {
        ...
        this._signalingClient = new KVSWebRTC.SignalingClient({
            ...
            requestSigner: {
                getSignedURL: async (signalingEndpoint: string, queryParams: any, date: Date | undefined) => {
    ...
    private async _checkWebRTCIngestionPath(): Promise<void> {
        if (!this._kinesisVideoClient) {
            ...
            this._kinesisVideoClient = new AWS.KinesisVideo({
                ...this._clientArgs,
                // @ts-ignore
                // this can be null, it's just not documented
    ...
    private async _getSignalingChannelEndpoints(kinesisVideoClient: AWS.KinesisVideo, arn: string, role: string, protocols: string[]): Promise<{ [key: string]: string }> {
